...
test('drawer and modal set focus, close with Escape, and restore their triggers', async ({ page }) => {
  ...
  const drawer = page.getByRole('dialog', { name: /当前设计师的数字人/ })
  ...
  const modalTrigger = page.getByRole('button', { name: '确认并沉淀', exact: true })
  await modalTrigger.click()
  const modal = page.getByRole('dialog', { name: '确认并沉淀新经验' })
