  ...
  return (
    <aside className="flex h-full w-[260px] shrink-0 flex-col border-r border-white/[0.06] bg-base">
      <div className="flex items-center gap-3 px-5 py-5">
        ...
        <div className="leading-tight"><div className="text-[15px] font-semibold text-white">我的数字人</div><div className="text-[11px] tracking-wide text-slate-500">AgentMesh</div></div>
      </div>
      ...
      <div className="border-t border-white/[0.06] p-3">
        <div className="mb-2 flex items-center gap-2 rounded-[10px] bg-surface-1 px-3 py-2"><span className="h-2 w-2 rounded-full bg-mint-400" /><span className="text-xs text-slate-400">当前空间</span><span className="ml-auto truncate text-xs font-medium text-slate-200">{bootstrap?.workspace.name}</span></div>
        <button type="button" onClick={onOpenProfile} aria-label="打开个人资料" className="flex min-h-10 w-full items-center gap-3 rounded-[10px] px-2 py-2 text-left transition-colors hover:bg-white/[0.04] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-mint-400/50"><Avatar name={user?.name ?? ''} size="md" tone="mint" /><span className="min-w-0 flex-1 leading-tight"><span className="block truncate text-sm font-medium text-slate-100">{user?.name}</span><span className="block truncate text-xs text-slate-500">{user?.role}</span></span><ChevronRight className="h-4 w-4 text-slate-600" /></button>
        {capabilities.length > 0 ? <button type="button" onClick={onOpenSettings} className="mt-1 flex min-h-10 w-full items-center gap-3 rounded-[10px] px-3 py-2 text-sm text-slate-400 transition-colors hover:bg-white/[0.04] hover:text-slate-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-mint-400/50"><Settings className="h-[18px] w-[18px]" />设置</button> : null}
        <button type="button" onClick={() => void logout().catch(() => undefined)} className="mt-1 flex min-h-10 w-full items-center gap-3 rounded-[10px] px-3 py-2 text-sm text-slate-400 transition-colors hover:bg-white/[0.04] hover:text-slate-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-mint-400/50"><LogOut className="h-[18px] w-[18px]" />{logoutError ? '重试退出' : '退出登录'}</button>
