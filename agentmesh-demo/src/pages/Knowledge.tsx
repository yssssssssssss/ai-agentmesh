...
function userMemoryCard(item: UserMemoryItem, projectName: string) {
  return {
...
export function Knowledge() {
  ...
  const memories = queries.memory.data?.items ?? []
  ...
  return (
    <div className="space-y-6">
      ...
      {tab === 'personal' ? (
        <section className="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-3" aria-label="个人知识">
          {[...personal, ...project, ...archive].map((item) => (
            <KnowledgeCard key={item.id} data={userMemoryCard(item, projectName)} />
          ))}
        ...
        <section className="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-3" aria-label="团队候选">
          {candidates.map((item) => (
            <KnowledgeCard
              key={item.id}
              accent="knowledge"
              data={{ id: item.id ?? item.title, title: item.title, summary: item.summary, memoryType: item.memory_type, scope: '待审核', project: projectName, updated: item.created_at }}
      ...
      <div className="flex items-center gap-2 text-xs text-slate-600">
        ...
        私有记忆只出现在个人知识；共享列表只展示服务端返回的 team_accepted 记录。
      </div>
